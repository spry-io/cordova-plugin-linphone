...
import com.sip.linphone.receivers.HookReceiver;
...
//    }
    ...
    public void login(String username, String password, String domain, CallbackContext callbackContext) {
        try {
            ...
            System.out.println( "ENTROU NO LinphoneManager  username: " + username + " domain: " + domain + " password: " + password);
//            username = getDisplayableUsernameFromAddress(username);
//            domain = getDisplayableUsernameFromAddress(domain);
            System.out.println( "ENTROU NO LinphoneManager tratados ->  username: " + username + " domain: " + domain );
            ...
            builder.setTransport(TransportType.Tcp);
            ...
            System.out.println( "Terminou o LinphoneManager Login! ");
    ...
    public void onRegistrationStateChanged(final Core lc, final ProxyConfig proxy, final RegistrationState state, final String message) {
        ...
        LOG.d(TAG, "New registration state [" + state + "]  | isNetworkReachable: " + LinphoneManager.getLc().isNetworkReachable());
    ...
    public String getDisplayableUsernameFromAddress(String sipAddress) {
...
//            LOG.d(TAG, "Error: error_network_unreachable");
//        }
//    }
...
//            if (mServiceContext.getResources().getBoolean("enable_call_notification"))
...
//            CallActivity.instance().refreshInCallActions();
    ...
    public String linphonerc_default(){
        return "[sip]\n" +
    ...
    public String linphonerc_factory(){
        return "\n" +
                "#\n" +
    ...
    public String assistant_create(){
        return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
    ...
    private void copyAssetsFromPackage() throws IOException {
        copyIfNotExist(linphonerc_default(), mConfigFile);
        copyFromPackage(linphonerc_factory(), new File(mLinphoneFactoryConfigFile).getName());
        copyIfNotExist(lpconfig(), mLPConfigXsd);
        copyFromPackage(assistant_create(), new File(mDynamicConfigFile).getName());
    }

    public void copyIfNotExist(String ressourceId, String target) throws IOException {
    ...
    public void copyFromPackage(String ressourceId, String target) throws IOException {
        FileOutputStream lOutputStream = mServiceContext.openFileOutput(target, 0);
        InputStream lInputStream = new ByteArrayInputStream(ressourceId.getBytes(Charset.forName("UTF-8")));
        int readByte;
...
//    }
    ...
    public boolean hasLinphoneAccount() {
        for (ProxyConfig proxyConfig : mLc.getProxyConfigList()) {
            if ("sip.linphone.org".equals(proxyConfig.getIdentityAddress().getDomain())) {
                return true;
            }
        }
        return false;
        ...
        if (result == VersionUpdateCheckResult.NewVersionAvailable) {
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    builder.setMessage("An update is available" + ": " + versionAv);
                    builder.setCancelable(false);
